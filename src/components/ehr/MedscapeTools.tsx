...
const drugDatabase: DrugEntry[] = [
  ...
  },
  {
    ...
    category: "Short-acting β2-agonist (SABA)",
    schedule: "Prescription",
...
const knownInteractions: InteractionResult[] = [
  {
    drug1: "Metformin", drug2: "Enalapril", severity: "monitor",
    mechanism: "ACE inhibitors may enhance hypoglycaemic effect",
    clinicalEffect: "Increased risk of hypoglycaemia",
    recommendation: "Monitor blood glucose more frequently when initiating or adjusting ACE inhibitor",
  },
  {
    drug1: "Enalapril", drug2: "Hydrochlorothiazide", severity: "significant",
    mechanism: "Additive hypotensive effect; risk of first-dose hypotension",
    clinicalEffect: "Excessive blood pressure reduction, dizziness, syncope",
    recommendation: "Start with low dose of one agent. Monitor BP closely. Consider withholding diuretic 2–3 days before starting ACE inhibitor",
  },
  {
    drug1: "Amoxicillin", drug2: "Metformin", severity: "minor",
    mechanism: "No significant pharmacokinetic interaction",
    clinicalEffect: "Minimal — may alter GI absorption slightly",
    recommendation: "No dose adjustment necessary. Monitor as usual",
  },
  {
    drug1: "Omeprazole", drug2: "Metformin", severity: "monitor",
    mechanism: "PPI-induced vitamin B12 malabsorption may compound metformin effect",
    clinicalEffect: "Increased risk of B12 deficiency with long-term co-use",
    recommendation: "Check B12 levels annually in patients on both long-term",
  },
  {
    drug1: "Salbutamol", drug2: "Hydrochlorothiazide", severity: "significant",
    mechanism: "Both agents can cause hypokalaemia",
    clinicalEffect: "Additive potassium-lowering effect; risk of arrhythmias",
    recommendation: "Monitor serum potassium. Consider potassium supplementation",
  },
];
...
interface CalculatorDef {
  ...
  description: string;
  category: string;
  fields: { label: string; unit: string; min?: number; max?: number; options?: string[] }[];
...
const calculators: CalculatorDef[] = [
  {
    ...
    category: "General",
    fields: [
      { label: "Weight", unit: "kg", min: 1, max: 500 },
      { label: "Height", unit: "cm", min: 30, max: 300 },
    ],
    calculate: (v) => {
  ...
  },
  {
    ...
    category: "Renal",
    fields: [
      { label: "Serum Creatinine", unit: "mg/dL", min: 0.1, max: 30 },
      { label: "Age", unit: "years", min: 18, max: 120 },
      { label: "Sex", unit: "", options: ["Female", "Male"] },
    ],
    calculate: (v) => {
  ...
  },
  {
    ...
    description: "Clinical prediction rule for deep vein thrombosis",
    ...
    calculate: (v) => {
      ...
      Object.values(v).forEach((val) => {
        if (String(val).includes("+1")) score += 1;
        if (String(val).includes("-2")) score -= 2;
      });
      let interp = "Low probability (score <1) — DVT unlikely, consider D-dimer";
      let color = "text-emerald-600";
  ...
  },
  {
    ...
    calculate: (v) => {
      ...
      let interp = "Score 0 — Low risk, anticoagulation not recommended";
      let color = "text-emerald-600";
  ...
  },
  {
    ...
    calculate: (v) => {
      ...
      let interp = "qSOFA 0–1 — Low risk of poor outcome";
      let color = "text-emerald-600";
  ...
  },
  {
    id: "apgar",
    name: "APGAR Score",
    description: "Neonatal assessment at 1 and 5 minutes",
    category: "Obstetrics",
    fields: [
    ...
    calculate: (v) => {
      ...
      let interp = "7–10: Normal — routine care";
      let color = "text-emerald-600";
  ...
  return (
    <>
      <Button
        variant="ghost"
        size="sm"
        className={`h-8 gap-1.5 text-xs shrink-0 ${open ? "bg-primary/10 text-primary" : ""}`}
        ...
        <SheetContent side="right" className="w-[650px] sm:max-w-[650px] p-0 flex flex-col">
          ...
          <div className="px-4 py-2">
            <div className="relative">
              ...
              <Input
                placeholder="Search drugs, conditions, topics and more..."
                value={searchQuery}
                onChange={(e) => { setSearchQuery(e.target.value); setSelectedDrug(null); }}
                className="pl-9"
              />
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            {selectedDrug ? (
              <DrugMonograph drug={selectedDrug} onBack={() => setSelectedDrug(null)} />
            ) : (
              <div className="p-2">
                {filtered.map((drug) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-3 py-3 rounded-md hover:bg-muted/80 transition-colors text-left"
                  >
...
function DrugMonograph({ drug, onBack }: { drug: DrugEntry; onBack: () => void }) {
  return (
    <div className="p-4 space-y-4">
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-3 mt-3">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Indication</CardTitle></CardHeader>
            <CardContent><p className="text-sm">{drug.indication}</p></CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Pregnancy</CardTitle></CardHeader>
            <CardContent><p className="text-sm">{drug.pregnancy}</p></CardContent>
          </Card>
        </TabsContent>

        <TabsContent value="dosing" className="mt-3">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Dosing</CardTitle></CardHeader>
            <CardContent><p className="text-sm">{drug.dosing}</p></CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="safety" className="space-y-3 mt-3">
          ...
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Side Effects</CardTitle></CardHeader>
            <CardContent>
        ...
        <TabsContent value="interactions" className="mt-3">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Known Interactions</CardTitle></CardHeader>
            <CardContent>
...
export function InteractionCheckerSheet() {
  ...
  const updateDrug = (idx: number, val: string) => {
    setDrugs((d) => d.map((v, i) => (i === idx ? val : v)));
    setChecked(false);
  };
        ...
        <SheetContent side="right" className="w-[600px] sm:max-w-[600px] p-0 flex flex-col">
          ...
          <div className="p-4 space-y-2">
            {drugs.map((drug, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    placeholder={`Enter medication ${idx + 1}`}
                    value={drug}
                  ...
                  <datalist id={`drug-suggestions-${idx}`}>
                    {getSuggestions(drug).map((s) => (
                      <option key={s} value={s} />
                    ))}
                  </datalist>
                </div>
                {drugs.length > 2 && (
                  <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => removeDrug(idx)}>
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="text-xs" onClick={addDrug}>
              ...
              <Button size="sm" className="text-xs" onClick={checkInteractions} disabled={drugs.filter((d) => d.trim()).length < 2}>
                Check Interactions
              </Button>
            </div>
          </div>
          ...
          <ScrollArea className="flex-1">
            {checked && results.length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  No known interactions between the entered medications
                </p>
              </div>
            )}
                    ...
                    <Card key={idx}>
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="flex items-center gap-2 text-sm font-medium">
                          <Badge variant="outline">{r.drug1}</Badge>
                          <span className="text-muted-foreground">+</span>
                          <Badge variant="outline">{r.drug2}</Badge>
                        </div>
                        <div className="space-y-2 text-sm">
                          ...
                          <div>
                            <span className="text-xs font-medium text-muted-foreground">Clinical Effect: </span>
                            <span>{r.clinicalEffect}</span>
                          </div>
                          <div className="bg-muted/50 rounded-md p-2.5">
                            <span className="text-xs font-semibold">Recommendation: </span>
                            <span className="text-xs">{r.recommendation}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
            ...
            {!checked && (
              <div className="p-8 text-center">
                <svg className="w-12 h-12 text-muted-foreground mx-auto mb-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <circle cx="8" cy="12" r="5" />
                  <circle cx="16" cy="12" r="5" />
                </svg>
                <p className="text-sm text-muted-foreground">
                  Enter two or more medications and click "Check Interactions"
                </p>
              </div>
            )}
          </ScrollArea>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-[550px] sm:max-w-[550px] p-0 flex flex-col">
          <SheetHeader className="p-4 pb-0">
            ...
            <p className="text-xs text-muted-foreground">Clinical decision support tools</p>
          </SheetHeader>
  ...
  return (
    <div className="p-4 space-y-4">
            ...
            <div key={field.label} className="flex items-center gap-3">
              ...
              {field.options ? (
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  />
                  {field.unit && (
                    <span className="text-xs text-muted-foreground w-14 shrink-0">{field.unit}</span>
                  )}
                </div>
              )}
            </div>
      ...
      {result && (
        <Card className="border-2">
          <CardContent className="p-4 text-center">
            ...
            <p className={cn("text-sm font-medium mt-2", result.color)}>
      ...
      <p className="text-[11px] text-muted-foreground text-center">
        Clinical calculators are for decision support only. Always verify with clinical judgment.
      </p>
    </div>
  );
}
...
export function ConditionsBrowserSheet() {
  ...
  const allConditions = conditionCategories.flatMap((c) =>
    c.conditions.map((cond) => ({ ...cond, categoryName: c.name }))
  );
                ...
                {conditionCategories.map((cat) => (
                  <div key={cat.name} className="mb-1">
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2.5 rounded-md hover:bg-muted/80 text-left"
                    >
                      <div className="flex items-center gap-2">
