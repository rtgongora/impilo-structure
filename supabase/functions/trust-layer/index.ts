...
const corsHeaders = {
  ...
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-facility-id, x-purpose-of-use",
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (method === "POST" && path === "/id/issue") {
      ...
      if (error) {
        console.error("Issue identity error:", error);
        return new Response(
          JSON.stringify({ error: "Failed to issue identity" }),
    ...
    if (method === "POST" && path === "/id/resolve/clinical") {
      ...
      if (!impiloId) {
        return new Response(
          JSON.stringify({ error: "impiloId required" }),
      ...
      if (error || !data || data.length === 0) {
        ...
        return new Response(
          JSON.stringify({ error: "Resolution failed" }),
      ...
      if (!result.consent_active && purposeOfUse !== "emergency") {
        ...
        return new Response(
          JSON.stringify({ error: "Consent required for access" }),
      ...
      await supabase.from("trust_layer_access_tokens").insert({
        token_hash: accessToken, // In production, hash this
        user_id: userId,
        subject_cpid: result.cpid,
        scope: "clinical",
        purpose_of_use: purposeOfUse,
    ...
    if (method === "POST" && path === "/id/resolve/registry") {
      ...
      // Rate limit (stricter for registry)
    ...
    if (method === "POST" && path === "/id/rotate/impiloId") {
      ...
      if (!healthId || !reason) {
        return new Response(
          JSON.stringify({ error: "healthId and reason required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    if (method === "POST" && path === "/id/offline/reconcile") {
      ...
      if (!Array.isArray(offlineCpids)) {
        return new Response(
          JSON.stringify({ error: "offlineCpids array required" }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    console.error("Trust Layer error:", error);
    return new Response(
